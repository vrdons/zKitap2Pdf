...
impl PeInfo {
    pub fn parse(raw: &[u8]) -> Result<Self, Error> {
        ...
        let num_sections =
            u16::from_le_bytes(raw[coff_header_offset + 2..coff_header_offset + 4].try_into().unwrap());

        let opt_header_size =
            u16::from_le_bytes(raw[coff_header_offset + 16..coff_header_offset + 18].try_into().unwrap());

        let opt_header_start = coff_header_offset + 20;

        let magic = u16::from_le_bytes(
            raw[opt_header_start..opt_header_start + 2].try_into().unwrap(),
        );
        ...
        for i in 0..num_sections as usize {
            ...
            let virtual_size = u32::from_le_bytes(raw[sec_off + 8..sec_off + 12].try_into().unwrap());
            let virtual_address =
            ...
            let raw_offset = u32::from_le_bytes(raw[sec_off + 20..sec_off + 24].try_into().unwrap());
